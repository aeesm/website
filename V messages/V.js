...
document.addEventListener("DOMContentLoaded", () => {
  const letter = document.getElementById("clover");

  const path = decodeURIComponent(window.location.pathname);
  console.log("Current path:", path);

  const match = path.match(/V\s*messages\/V\s*\((\d+)\)\.html/);
...
submitBtn.addEventListener('click', function() {
  ...
  localStorage.setItem(setPrefix + '_savedMessage_' + i, message);
  localStorage.setItem(setPrefix + '_savedName_' + i, name);
});
...
editBtn.addEventListener('click', function() {
  ...
  messageInput.value = localStorage.getItem(setPrefix + '_savedMessage_' + i) || '';
  nameInput.value = localStorage.getItem(setPrefix + '_savedName_' + i) || '';
});
